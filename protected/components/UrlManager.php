<?php
/**
 * An customized url manager that routes between http and https according to settings.
 * <p></p>
 * Originated from this yii tutorial: http://www.yiiframework.com/wiki/407/url-management-for-websites-with-secure-and-nonsecure-pages/
 * @author Wenbin
 *
 */
class UrlManager extends CUrlManager
{
	...
	protected function isSecureRoute($route)
	{
		...
		/* var_dump($route);
		var_dump(($pos = strrpos($route, '/')) !== false
			&& isset($this->_secureMap[substr($route, 0, $pos)]));
		var_dump(substr($route, 0, $pos));
		var_dump($pos);
		die();  */
		...
		if (isset($this->_secureMap[$route])) {
			...
			return ($pos = strrpos($route, '/')) !== false
			&& isset($this->_secureMap[substr($route, 0, $pos)]);
		}
	}
}